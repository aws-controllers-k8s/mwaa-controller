...
// +kubebuilder:rbac:groups=mwaa.services.k8s.aws,resources=environments,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=mwaa.services.k8s.aws,resources=environments/status,verbs=get;update;patch

var lateInitializeFieldNames = []string{"AirflowVersion", "EndpointManagement", "EnvironmentClass", "LoggingConfiguration", "MaxWebservers", "MaxWorkers", "MinWebservers", "MinWorkers", "Schedulers", "WebserverAccessMode", "WeeklyMaintenanceWindowStart"}
...
func (rm *resourceManager) incompleteLateInitialization(
	...
	if ko.Spec.AirflowVersion == nil {
		return true
	}
	if ko.Spec.EndpointManagement == nil {
		return true
	}
	if ko.Spec.EnvironmentClass == nil {
		return true
	}
	if ko.Spec.LoggingConfiguration == nil {
		return true
	}
	if ko.Spec.MaxWebservers == nil {
		return true
	}
	if ko.Spec.MaxWorkers == nil {
		return true
	}
	if ko.Spec.MinWebservers == nil {
		return true
	}
	if ko.Spec.MinWorkers == nil {
		return true
	}
	if ko.Spec.Schedulers == nil {
		return true
	}
	if ko.Spec.WebserverAccessMode == nil {
		return true
	}
	if ko.Spec.WeeklyMaintenanceWindowStart == nil {
		return true
	}
	return false
}
...
func (rm *resourceManager) lateInitializeFromReadOneOutput(
	...
	if observedKo.Spec.AirflowVersion != nil && latestKo.Spec.AirflowVersion == nil {
		latestKo.Spec.AirflowVersion = observedKo.Spec.AirflowVersion
	}
	if observedKo.Spec.EndpointManagement != nil && latestKo.Spec.EndpointManagement == nil {
		latestKo.Spec.EndpointManagement = observedKo.Spec.EndpointManagement
	}
	if observedKo.Spec.EnvironmentClass != nil && latestKo.Spec.EnvironmentClass == nil {
		latestKo.Spec.EnvironmentClass = observedKo.Spec.EnvironmentClass
	}
	...
	if observedKo.Spec.MaxWebservers != nil && latestKo.Spec.MaxWebservers == nil {
		latestKo.Spec.MaxWebservers = observedKo.Spec.MaxWebservers
	}
	if observedKo.Spec.MaxWorkers != nil && latestKo.Spec.MaxWorkers == nil {
		latestKo.Spec.MaxWorkers = observedKo.Spec.MaxWorkers
	}
	if observedKo.Spec.MinWebservers != nil && latestKo.Spec.MinWebservers == nil {
		latestKo.Spec.MinWebservers = observedKo.Spec.MinWebservers
	}
	if observedKo.Spec.MinWorkers != nil && latestKo.Spec.MinWorkers == nil {
		latestKo.Spec.MinWorkers = observedKo.Spec.MinWorkers
	}
	if observedKo.Spec.Schedulers != nil && latestKo.Spec.Schedulers == nil {
		latestKo.Spec.Schedulers = observedKo.Spec.Schedulers
	}
	if observedKo.Spec.WebserverAccessMode != nil && latestKo.Spec.WebserverAccessMode == nil {
		latestKo.Spec.WebserverAccessMode = observedKo.Spec.WebserverAccessMode
	}
	if observedKo.Spec.WeeklyMaintenanceWindowStart != nil && latestKo.Spec.WeeklyMaintenanceWindowStart == nil {
		latestKo.Spec.WeeklyMaintenanceWindowStart = observedKo.Spec.WeeklyMaintenanceWindowStart
	}
